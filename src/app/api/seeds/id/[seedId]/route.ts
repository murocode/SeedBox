...
export async function DELETE(request: Request, { params }: { params: Promise<{ seedId: string }> }) {
  try {
    ...
    await prisma.$transaction([
      prisma.like.deleteMany({ where: { seedId: id } }),
      prisma.favorite.deleteMany({ where: { seedId: id } }),
      prisma.report.deleteMany({ where: { targetSeedId: id } }),
      prisma.seed.delete({ where: { id } })
    ])
